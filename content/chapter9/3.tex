...
span是具有引用语义的视图。从这个意义上讲，其行为就像指针:若span是const，并不意味着该span所引用的元素也是const。
...
这里的问题不在于std::span被破坏了;问题是，像std::cbegin()和std::ranges::cbegin()这样的函数破坏了具有引用语义的集合(例如视图)。
...
\mySubsubsection{9.3.4}{在泛型代码中使用span作为参数}
