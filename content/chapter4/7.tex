...
检查假设的工作只针对概念进行评估。若没有使用任何概念，则具有不同约束的重载不明确:
...
现在，当我们为这两个概念重载两个函数，并传递一个同时适合这两个概念的对象时，这并不具有二义性:
...
要正确地实现假设，必须非常小心。一个很好的例子是std::same\_as概念的实现，它检查两个模板形参是否具有相同的类型。
